...
class ajax_gitlab extends AWS_CONTROLLER
{
    ...
    public function setup()
    {
        ...
        {
            H::ajax_json_output(AWS_APP::RSM(null, -1, AWS_APP::lang()->_t('本站未开通 GitLab 登录')));
        }
    }
    ...
    public function register_action()
    {
        ...
        {
            H::ajax_json_output(AWS_APP::RSM(null, -1, AWS_APP::lang()->_t('GitLab 账号信息不存在')));
        }

        if ($this->model('openid_gitlab')->get_gitlab_user_by_id(AWS_APP::session()->gitlab_user['id']))
        {
            H::ajax_json_output(AWS_APP::RSM(null, -1, AWS_APP::lang()->_t('此 GitLab 账号已被绑定')));
        }
        ...
        else
        {
            ...
            if (AWS_APP::session()->gitlab_user['email'] == $_POST['email'] AND AWS_APP::session()->gitlab_user['verified_email'] == true)
            {
        ...
        $this->model('openid_gitlab')->bind_account(AWS_APP::session()->gitlab_user, $uid);

        if (AWS_APP::session()->gitlab_user['picture'])
        {
            $this->model('account')->associate_remote_avatar($uid, AWS_APP::session()->gitlab_user['picture']);
        }
